...
export async function POST(req: Request) {
  ...
  const { cliente, itens, observacao, clienteId, vendedorNome, transportadoraNome, formaPagamento, empresa } = body;
  const companyId: string = empresa === "ecopro" ? "ecopro" : "nyer";
  ...
  const itensFormatados = await Promise.all(
    itens.map(async (i: { sku: string | null; nome: string; quantidade: number; valor_unitario: number }) => {
      ...
      try {
        const res = await tinyFetch(`/produtos?codigo=${encodeURIComponent(i.sku)}`, {}, companyId);
        if (res.ok) {
  ...
  if (!idContato) {
    ...
    try {
      const sres = await tinyFetch(`/contatos?nome=${encodeURIComponent(cliente.nome)}&limit=5`, {}, companyId);
      if (sres.ok) {
  ...
  if (vendedorNome) {
    try {
      const vres = await tinyFetch(`/vendedores?nome=${encodeURIComponent(vendedorNome)}&limit=5`, {}, companyId);
      if (vres.ok) {
