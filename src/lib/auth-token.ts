...
export const AUTH_COOKIE = "exx_auth";

export function authCredentials(): { username: string; password: string } {
  return {
    username: process.env.APP_USERNAME || "exx",
    ...
    password: process.env.APP_PASSWORD || "ExxLog@2026",
  };
}
...
export async function computeAuthToken(username: string, password: string): Promise<string> {
  const data = new TextEncoder().encode(`${username}:${password}:exx-pos-venda`);
